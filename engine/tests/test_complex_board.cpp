...
void test_board_scenario_8() {
    cout << "\n=== Test: Example Complex Board 8 ===" << endl;
    ...
    Rack rack("AAAAAAAAABBCCDDDEEEEEEEEEEEEEEEFFGGHHIIIIIIIIJKLLLLLMMMNNNNNNOOOOOOPPQRRRRRRSSSSSSTTTTTTUUUUUUVVWXYZ??");
    MoveGenerator gen(board, rack, dawg);

    auto top_moves = gen.getBestMove();
    bool found_whiskey_144_pts = false;
    for (const auto& move : top_moves) {
        if (move.getWord() == "WHISKEY" && move.getScore() == 144) {
            found_whiskey_144_pts = true;
            break;
        }
    }
    assert_true(found_whiskey_144_pts, "Found WHISKEY for 144 pts");
}

void test_board_scenario_9() {
    cout << "\n=== Test: Example Complex Board 9 ===" << endl;
    ...
    Rack rack("AAAAAAAAABBCCDDDEEEEEEEEEEEEEEFFGGHIIIIIIIJLLLLLMMMNNNNNNOOOOOOPPQRRRRRRSSSSSTTTTTTUUUUUUVVXZ??");
    MoveGenerator gen(board, rack, dawg);

    auto top_moves = gen.getBestMove();
    bool found_desaxez_272_pts = false;
    for (const auto& move : top_moves) {
        if (move.getWord() == "DESAXEZ" && move.getScore() == 272) {
            found_desaxez_272_pts = true;
            break;
        }
    }
    assert_true(found_desaxez_272_pts, "Found DESAXEZ for 272 pts");
}
...
int main() {
    ...
    test_board_scenario_8();
    test_board_scenario_9();
    // TIME_TEST("test whiskey", test_board_scenario_8);
